...
/*
CREATE PROCEDURE excuteSeckill(IN fadeId INT,IN fadeSeckillId INT,IN fadeUserId INT,IN fadeUserPhone VARCHAR (15),IN fadeKillTime TIMESTAMP ,OUT fadeResult INT)
  BEGIN
    DECLARE insert_count INT DEFAULT 0;
    START TRANSACTION ;
    INSERT ignore success_killed(id,seckill_id,user_id,user_phone,state,create_time) VALUES(fadeId,fadeSeckillId,fadeUserId,fadeUserPhone,0,fadeKillTime);
    SELECT ROW_COUNT() INTO insert_count;
    IF(insert_count = 0) THEN
      ROLLBACK ;
      SET fadeResult = -1;
